...
def fetch_text(url: str) -> str:
    ...
    request = urllib.request.Request(
        f"{url}{separator}nocache={time.time_ns()}",
        headers={"Cache-Control": "no-cache", "Pragma": "no-cache", "User-Agent": "genesis-validator-node/1.0"},
    )
...
def main() -> None:
    ...
    try:
        ...
        head = genesis_anchor
        height = 0
    core = {"peer_id": NODE_ID, "repository": NODE_REPOSITORY, "source_repo": SOURCE_REPO, "network": "gden/0.1", "head": head, "height": height, "genesis_anchor": genesis_anchor, "observed_at": datetime.now(timezone.utc).isoformat()}
    ...
    if private_key_pem:
        ...
        attestation.update({"attestation_mode": "ed25519-signed", "signature_algorithm": "ed25519", "signature": signature, "signed_payload_sha256": sha256_bytes(canonical_bytes(core)), "public_key_sha256": public_key_sha256})
